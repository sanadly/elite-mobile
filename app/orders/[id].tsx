...
export default function OrderDetailScreen() {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.date}>{format(new Date(order.created_at), 'MMMM dd, yyyy • HH:mm')}</Text>

      {/* Order Timeline */}
      <Card style={styles.timelineCard}>
        <Text style={styles.sectionTitle}>{t('orders.detail.order_status')}</Text>
                  ...
                  <View style={styles.timelineLeft}>
                    ...
                    >
                      <Ionicons
                        ...
                        size={20}
                        color={isCompleted ? colors.primary.foreground : colors.muted.foreground}
                      />
                    </View>
                    {!isLast && (
                      <View
                  ...
                  <View style={styles.timelineRight}>
                    ...
                    {order.status_history?.[status] && (
                      <Text style={styles.timelineTime}>
                        {format(new Date(order.status_history[status]), 'MMM dd, HH:mm')}
                      </Text>
                    )}
                  </View>
                </View>
      ...
      <Card style={styles.card}>
        <Text style={styles.sectionTitle}>{t('orders.detail.shipping_info')}</Text>
        <View style={styles.infoRow}>
          <Ionicons name="person-outline" size={20} color={colors.muted.foreground} />
          <Text style={styles.infoText}>{order.shipping_address.fullName}</Text>
        </View>
        <View style={styles.infoRow}>
      ...
      {/* Order Items */}
      <Card style={styles.card}>
        <Text style={styles.sectionTitle}>{t('orders.detail.order_items')}</Text>
        {order.items.map((item) => (
          <View key={item.id} style={styles.itemRow}>
            ...
            <View style={styles.itemInfo}>
              ...
              <Text style={styles.itemQuantity}>{t('orders.detail.quantity', { quantity: item.quantity })}</Text>
            </View>
      ...
      {/* Order Summary */}
      <Card style={styles.card}>
        <Text style={styles.sectionTitle}>{t('orders.detail.order_summary')}</Text>

        <View style={styles.summaryRow}>
          <Text style={styles.summaryLabel}>{t('checkout.subtotal')}</Text>
          <Text style={styles.summaryValue}>
            €{(order.total_eur - order.shipping_fee + (order.discount_amount || 0)).toFixed(2)}
          </Text>
        </View>
        ...
        {order.discount_amount && order.discount_amount > 0 && (
          <View style={styles.summaryRow}>
            <Text style={[styles.summaryLabel, styles.discountLabel]}>
              {t('checkout.discount')} {order.coupon_code && `(${order.coupon_code})`}
            </Text>
        ...
        {order.deposit_amount && order.deposit_amount > 0 && (
          <>
            ...
            <View style={styles.summaryRow}>
              <Text style={styles.depositLabel}>{t('checkout.deposit')}</Text>
        ...
        <View style={styles.paymentMethodRow}>
          ...
          <Text style={styles.paymentMethodText}>{t('orders.detail.payment_method')}</Text>
        </View>
      </Card>

      {/* Help Section */}
      <Card style={styles.helpCard}>
        <Text style={styles.helpTitle}>{t('orders.detail.help_title')}</Text>
        <Text style={styles.helpText}>
          {t('orders.detail.help_text')}
        </Text>
      </Card>
