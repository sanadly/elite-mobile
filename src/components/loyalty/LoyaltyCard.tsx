    ...
    <View style={styles.container}>
      ...
      <View style={styles.progressContainer}>
        ...
        <View style={styles.progressInfo}>
          <Text style={styles.spendLabel}>{t('loyalty.total_spend')}</Text>
          <Text style={styles.spendAmount}>{'\u20AC'}{totalSpend.toFixed(0)}</Text>

          {progress.nextTier && (
            <>
              <View style={styles.divider} />
              <Text style={styles.nextTierLabel}>{t('loyalty.next_tier')}</Text>
              <View style={styles.nextTierRow}>
                <Ionicons
                  name={TIER_ICONS[progress.nextTier]}
                  size={18}
                  color={TIER_COLORS[progress.nextTier]}
                />
                <Text style={[styles.nextTierName, { color: TIER_COLORS[progress.nextTier] }]}>
                  {getTierName(progress.nextTier)}
                </Text>
              </View>
              <Text style={styles.remainingAmount}>
                {t('loyalty.unlock', { amount: progress.amountToNextTier.toFixed(0) })}
              </Text>
            </>
          )}
      ...
      <View style={styles.benefitsContainer}>
        <Text style={styles.benefitsTitle}>{t('loyalty.benefits.title')}</Text>

        <View style={styles.benefitRow}>
          <Ionicons
            name={currentTier === 'classic' ? 'close-circle' : 'checkmark-circle'}
            size={20}
            color={currentTier === 'classic' ? colors.muted.foreground : colors.status.success.text}
          />
          <Text
            style={[
              styles.benefitText,
              currentTier === 'classic' && styles.benefitDisabled,
            ]}
          >
            {t('loyalty.benefits.free_shipping')}
          </Text>
        </View>
        ...
        {currentTier === 'black' && (
          <View style={styles.benefitRow}>
            <Ionicons name="checkmark-circle" size={20} color={colors.status.success.text} />
            <Text style={styles.benefitText}>{t('loyalty.benefits.priority_support')}</Text>
          </View>
        )}
      </View>
    </View>
